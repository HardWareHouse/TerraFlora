<template>
    <div class="fixed inset-0 z-50 flex items-center justify-center bg-gray-800 bg-opacity-50">
      <div class="bg-white p-6 rounded shadow-lg w-full max-w-md">
        <h2 class="text-2xl font-bold mb-4">{{ product ? 'Éditer le Produit' : 'Ajouter un Produit' }}</h2>
        <form @submit.prevent="saveProduct">
          <div class="mb-4">
            <label for="nom" class="block text-sm font-medium">Nom</label>
            <input type="text" id="nom" v-model="form.nom" required class="mt-1 p-2 border rounded-md w-full bg-gray-100" />
          </div>
          <div class="mb-4">
            <label for="description" class="block text-sm font-medium">Description</label>
            <textarea id="description" v-model="form.description" class="mt-1 p-2 border rounded-md w-full bg-gray-100"></textarea>
          </div>
          <div class="mb-4">
          ...
          <div class="mb-4">
            ...
            <input type="number" id="stock" v-model="form.stock" required class="mt-1 p-2 border rounded-md w-full bg-gray-100" />
          </div>
          <div class="mb-4">
            <label for="marque" class="block text-sm font-medium">Marque</label>
            <select id="marque" v-model="form.marque" required class="mt-1 p-2 border rounded-md w-full bg-gray-100">
          ...
          <div class="mb-4">
            ...
            <select id="couleur" v-model="form.couleur" required class="mt-1 p-2 border rounded-md w-full bg-gray-100">
          ...
          <div class="mb-4">
            ...
            <select id="taille" v-model="form.taille" required class="mt-1 p-2 border rounded-md w-full bg-gray-100">
          ...
          <div class="mb-4">
            <label for="isPromotion" class="block text-sm font-medium">Promotion</label>
            <input type="checkbox" id="isPromotion" v-model="form.isPromotion" class="mt-1 p-2 border rounded-md bg-gray-100" />
          </div>
          <div class="mb-4" v-if="form.isPromotion">
            ...
            <input type="number" id="pourcentagePromotion" v-model="form.pourcentagePromotion" required class="mt-1 p-2 border rounded-md w-full bg-gray-100" />
          </div>
          <div class="mb-4">
  ...
  const saveProduct = async () => {
    try {
      if (props.product) {
        await axios.put(`http://localhost:8000/product/${props.product.id}`, form.value);
      } else {
        await axios.post('http://localhost:8000/product', form.value);
      }
      emit('save');
      emit('close');
    } catch (error) {
